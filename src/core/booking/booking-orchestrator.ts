...
export async function handleTravellerBookingMessage(
  ...
  if (analysis.intent === "HUMAN_HANDOFF") {
    ...
    // An operator-authored handoff line, or a knowledge answer if the
    // handoff was actually a policy question we can answer.
    ...
    return composeReplyResult({
      ...
      deterministicReply: pack?.escalation.handoffMessage ?? composeBookingBrainReply(analysis),
  ...
  if (analysis.intent === "GENERAL_QUESTION") {
    ...
    // No matching answer to a policy-shaped question → hand to a human
    // rather than let Kai improvise a policy the operator never gave.
    if (
      pack &&
